...
void QTriangulatingStroker::moveTo(const qreal *pts)
{
    ...
    case Qt::FlatCap:
        if (invisibleJump) {
            m_vertices.push_back(m_cx + m_nvx);
            m_vertices.push_back(m_cy + m_nvy);
        }
        break;
    case Qt::SquareCap: {
        float sx = m_cx - m_nvy;
        float sy = m_cy + m_nvx;
        if (invisibleJump) {
            m_vertices.push_back(sx + m_nvx);
            m_vertices.push_back(sy + m_nvy);
        }
...
void QTriangulatingStroker::join(const qreal *pts)
{
    ...
    case Qt::MiterJoin: {
        ...
        if ((ix - px) * (ix - px) + (iy - py) * (iy - py) <= m_miter_limit * m_miter_limit) {
            m_vertices.push_back(ix);
            m_vertices.push_back(iy);
            m_vertices.push_back(ix);
            m_vertices.push_back(iy);
        }
        // else
...
void QTriangulatingStroker::endCap(const qreal *)
{
    ...
    case Qt::RoundCap: {
        ...
        int end = points.size() / 2;
        while (front != end) {
            m_vertices.push_back(points[2 * end - 2]);
            m_vertices.push_back(points[2 * end - 1]);
            --end;
            if (front == end)
                break;
            m_vertices.push_back(points[2 * front + 0]);
            m_vertices.push_back(points[2 * front + 1]);
            ++front;
        }
        break; }
